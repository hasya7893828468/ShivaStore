...
const GroceryManager: React.FC = () => {
  ...
  useEffect(() => {
    const fetchGroceries = async () => {
      try {
        const res = await axios.get("http://192.168.144.2:5001/api/groceries");
  ...
  const handleAddGrocery = async () => {
    ...
    try {
      const res = await axios.post("http://192.168.144.2:5001/api/groceries", formData, {
  ...
  const handleDeleteGrocery = async (id: string) => {
    Alert.alert("Confirm Delete", "Are you sure you want to delete this grocery item?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`http://192.168.144.2:5001/api/groceries/${id}`);
